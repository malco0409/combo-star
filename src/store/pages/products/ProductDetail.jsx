...
export default function ProductDetail() {
  ...
  // collections faylida kalitlar uzbekcha — titleKey dan uzbekcha nom olamiz
  const uzTitle = product?.titleKey
  ...
  const collectionKey = uzTitle ? collectionKeyMap[uzTitle] : product?.title;
  const productCollection = collections[collectionKey] || collections[product?.title];
  ...
  const color = selectedColorObj ? t(selectedColorObj.nameKey) : "";
            ...
            {productCollection.type === "colors_only" && (
              <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-4">
                {productCollection.items.map((item) => (
                  <div key={item.id}
                    onClick={() => navigate("/collection", { state: { product, collection: { ...item, colors: productCollection.items.map(i => ({ name: i.name, nameKey: i.nameKey, image: i.image, price: i.price })) } } })}
                    ...
                    {item.image ? (
                      <img src={item.image} alt={item.nameKey ? t(item.nameKey) : item.name} className="w-full h-32 object-cover group-hover:scale-105 transition-transform duration-300" />
                    ) : (
                ...
                {productCollection.items.map((col) => (
                  <div key={col.id}
                    ...
                    <div className="grid grid-cols-3 sm:grid-cols-4 lg:grid-cols-6 gap-3">
                      {col.colors.map((c) => (
                        <div key={c.name}
                          onClick={() => navigate("/collection", { state: { product, collection: col, selectedColor: c.name } })}
                          ...
                          {c.image ? (
                            <img src={c.image} alt={c.nameKey ? t(c.nameKey) : c.name} className="w-full h-20 object-cover group-hover:scale-105 transition-transform duration-300" />
                          ) : (
